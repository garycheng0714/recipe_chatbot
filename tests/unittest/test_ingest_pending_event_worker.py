...
@pytest.mark.asyncio
async def test_ingest_pending_event_call_queue_task_done(queue, mock_session_factory):
    mock_service = MagicMock()
    mock_service.ingest_pending_url = AsyncMock()

    worker = IngestPendingEventWorker(mock_service, queue, mock_session_factory, MagicMock())
...
@pytest.mark.asyncio
async def test_ingest_pending_event_call_queue_task_done_when_exception_catch_exception(queue, mock_session_factory):
    mock_service = MagicMock()
    mock_service.ingest_pending_url = AsyncMock(side_effect=Exception("boom!"))
