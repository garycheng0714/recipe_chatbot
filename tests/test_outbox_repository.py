...
async def test_insert_event_creates_pending_event(repo, session, sample_recipe):
    await repo.insert_event(sample_recipe)
    await session.flush()
...
async def test_insert_event_idempotent(repo, session, sample_recipe):
    ...
    await repo.insert_event(sample_recipe)
    await repo.insert_event(sample_recipe)  # 第二次應該被 on_conflict_do_nothing 擋掉
    await session.flush()
...
async def test_claim_event_success(repo, session, sample_recipe):
    ...
    claimed = await repo.claim_event(event_id)
...
async def test_claim_event_returns_none_if_already_processing(repo, session, sample_recipe):
    ...
    event_id = str(repo.make_event_id(sample_recipe.id, "recipe.created"))
    first = await repo.claim_event(event_id)
    second = await repo.claim_event(event_id)  # 已經是 PROCESSING
...
async def test_claim_event_concurrent(engine, sample_recipe, outbox_cleaner):
    ...
    async with SessionFactory() as s:
        async with s.begin():
            await OutboxRepository(s).insert_event(sample_recipe)
    ...
    async def try_claim():
        async with SessionFactory() as s:
            async with s.begin():
                return await OutboxRepository(s).claim_event(event_id)
...
async def test_mark_event_completed(repo, session, sample_recipe):
    ...
    event_id = str(repo.make_event_id(sample_recipe.id, "recipe.created"))
    await repo.claim_event(event_id)
    await repo.mark_event_completed(event_id)
    await session.flush()
...
async def test_reset_stale_events(repo, session, sample_recipe):
    ...
    await repo.reset_stale_events(timeout_minutes=30)
    await session.flush()
