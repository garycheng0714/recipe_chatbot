...
async def test_embedding_chunks(chunks, mock_session_factory):
    ...
    qdrant = MagicMock()
    qdrant.upsert_batch_recipe = AsyncMock()
    ...
    qdrant.upsert_batch_recipe.assert_called_once()

    args, _ = qdrant.upsert_batch_recipe.call_args
    embed_models = args[0]
...
async def test_embeddings_more_chunks(more_chunks, mock_session_factory):
    ...
    yt_repo.fetch_chunks.assert_called_once()
    assert qdrant.upsert_batch_recipe.call_count == 2
...
async def test_embeddings_zero_chunk(more_chunks, mock_session_factory):
    ...
    yt_repo.fetch_chunks.assert_called_once()
    qdrant.upsert_batch_recipe.assert_not_called()